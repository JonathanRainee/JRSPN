...
public class TransAdapter extends RecyclerView.Adapter<TransAdapter.TransVH> {
    ...
    public void onBindViewHolder(@NonNull TransVH holder, int position) {
        ...
        Medicine med = Medicine.getMedicine(trans.get(holder.getAdapterPosition()).getMedicineid());
        ...
        holder.goUpdate.setOnClickListener(view -> {
            if (holder.updQty.getText().toString() == null || holder.updQty.getText().toString().isEmpty() || Integer.parseInt(holder.updQty.getText().toString())<=0){
                err.show();
            }
            else {
//                Transaction.UPDATE_TRANS_QTY(trans.get(holder.getAdapterPosition()).getID(), Integer.parseInt(holder.updQty.getText().toString()));
                Transaction.updateTransaction(trans.get(holder.getAdapterPosition()).getID(), Integer.parseInt(holder.updQty.getText().toString()));
                notifyItemChanged(holder.getAdapterPosition(), 1);
//                trans = Transaction.GET_TRANSACTIONS_USERID(User.curr.getID());
                trans = Transaction.getTrans(User.curr.getID());
                succ.show();
            }
        });
...
//            Transaction.DELETE_TRANS(trans.get(position).getID());
            Transaction.deleteTransaction(trans.get(position).getID());
